...
import numpy as np
import math
from pipeline_oak_user_v2 import daiPipeline
...
class oakPipeline:
    def __init__(self):
        ...
        self.b_hand_yolo = rospy.get_param('~hand_yolo', False)
        ...
        self.sub_rec = rospy.Subscriber(self.node_name+"/set_hand_yolo", Bool, self.cb_hand_yolo)
        ...
        # Hand yolo
        self.pub_hand_yolo_detections = rospy.Publisher(self.node_name+"/hands/signs/detections", SpatialDetectionArray, queue_size=10)
        self.pub_hand_yolo_marker = rospy.Publisher(self.node_name+"/hands/signs/markers", MarkerArray, queue_size=10)
        # User
        ...
        self.hand_yolo_detectionArray = SpatialDetectionArray()
        self.hand_yolo_markerArray = MarkerArray()
        ...
        self.flag_hand_yolo_marker = False
    ...
    def cb_hand_yolo(self, data):
        self.b_hand_yolo = data.data
    ...
    def loop(self):
        ...
        while not rospy.is_shutdown():
            ...
            frame, depth_frame, facerec_result, hands, signs_results = self.oak.next_frame()
            ...
            ''' HAND YOLO DETECTION AND SIGN RECOGNITION '''
            if frame is not None and signs_results is not None:
                self.hand_yolo_detectionArray.detections.clear()
                self.hand_yolo_detectionArray.header.stamp = rospy.Time.now()
                self.hand_yolo_detectionArray.header.frame_id = self.oak_frame+"_right_camera_optical_frame"
                self.hand_yolo_markerArray.markers.clear()
                marker_id = 0
                for detection in signs_results:
                    self.hand_yolo_detectionArray.detections.append(objDet_msg(self.oak, detection))
                    ...
                    hand_yolo_marker = Marker()
                    hand_yolo_marker.header.frame_id = self.oak_frame+"_right_camera_optical_frame"
                    hand_yolo_marker.id = marker_id
                    hand_yolo_marker.type = hand_yolo_marker.SPHERE
                    hand_yolo_marker.lifetime = rospy.Duration.from_sec(1)
                    #hand_yolo_marker.action = hand_yolo_marker.ADD
                    hand_yolo_marker.scale.x = 0.05
                    hand_yolo_marker.scale.y = 0.05
                    hand_yolo_marker.scale.z = 0.05
                    hand_yolo_marker.color.r = 0.0
                    hand_yolo_marker.color.g = 0.0
                    hand_yolo_marker.color.b = 1.0
                    hand_yolo_marker.color.a = 1.0
                    hand_yolo_marker.pose.orientation.w = 1.0
                    hand_yolo_marker.pose.position.x = detection.spatialCoordinates.x/1000
                    hand_yolo_marker.pose.position.y = -detection.spatialCoordinates.y/1000
                    hand_yolo_marker.pose.position.z = detection.spatialCoordinates.z/1000
                    self.hand_yolo_markerArray.markers.append(hand_yolo_marker)
                    marker_id = marker_id + 1
                
                self.flag_hand_yolo_marker = True
            else:
                self.flag_hand_yolo_marker = False
            ...
            if self.flag_hand_yolo_marker:
                self.pub_hand_yolo_marker.publish(self.hand_yolo_markerArray)
                self.pub_hand_yolo_detections.publish(self.hand_yolo_detectionArray)                   
